...
for SITE in ${SITES[*]}
  do
    ...
    /usr/local/bin/b2 sync --keepDays ${B2_KEEP} ${SITE_BACKUP_DIR}/${SITE}/ b2://${B2_BACKET}/${SITE}/ >> ${SCRIPT_DIR}/log.b2.out
  done
...
for DB in ${DBS[*]}
  do
    ...
    /usr/local/bin/b2 sync --keepDays ${B2_KEEP} ${DB_BACKUP_DIR}/${DB}/ b2://${B2_BACKET}/${DB}/ >> ${SCRIPT_DIR}/log.b2.out
  done
